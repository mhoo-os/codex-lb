...
proves that prior assistant or tool output is retained before the new input, the
service MUST submit that complete payload on the fresh upstream bridge without
injecting the durable `previous_response_id`. Otherwise, when the request needs
the durable response anchor to represent prior context, the service MUST
preserve existing durable-anchor reattach behavior. The service MUST NOT replay
a request after uncertain upstream acceptance.
...
session ID, owner account, latest response ID, stored count, and stored
fingerprint. The proof MUST be created only by the count, fingerprint, and
retained-output verifier; it MUST NOT be accepted from a caller, persisted,
deserialized, or ordinarily constructed, and mutation or durable-state
substitution MUST invalidate it.
