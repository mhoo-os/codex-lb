...
5. **Bound v1.23 upstream close separately.** The v1.23 backport does not
contain the later upstream-close cleanup helper. It therefore adds the same
tracked-close pattern locally, with a fixed 250 ms cap, so a close handshake
that stalls cannot consume the scope-finalization allowance or prevent request
and lease release. The task remains tracked for persistence drain after that
cap.
